    ...
    public void onSlashCommand(@Nonnull SlashCommandEvent event) {
        if (event.getName().equals("stop")) {
            ...
            if (!selfVoiceState.inVoiceChannel()) {
                ...
                ebd.addField("Unable To Stop", "I am not connected to a voice channel\nConnect to a VC and run /join-vc, then try again", true);
            ...
            if (!memberVoiceState.inVoiceChannel()) {
                ebd.setColor(Color.blue);
                ebd.addField("Unable To Stop", "You must be in a VC", true);
                event.getHook().editOriginalEmbeds(ebd.build()).queue();
                return;
            }
            if (!memberVoiceState.getChannel().equals(selfVoiceState.getChannel())) {
                ebd.setColor(Color.blue);
                ebd.addField("Unable To Stop", "You must be in the same VC as me", true);
                event.getHook().editOriginalEmbeds(ebd.build()).queue();
                return;
            }
            ...
            ebd.addField("Stopped", "Cleared the queue and stopped music", true);
